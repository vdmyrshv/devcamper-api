...
exports.createBootcamp = async (req, res, next) => {
	...
	//check for published bootcamp (publisher should only be able to create one bootcamp)
	...
	if (publishedBootcamp && req.user.role !== 'admin') {
		return next(
			new ErrorResponse(
				'you are not authorized to add multiple bootcamps',
				400
			)
		)
	}
...
exports.uploadBootcampPhoto = async (req, res, next) => {
	...
	if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
		return next(
			new ErrorResponse(
				`User ${req.user.id} is not authorized to update photo for this bootcamp`,
				401
			)
		)
	}
